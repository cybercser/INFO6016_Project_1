...
int main(int argc, char** argv)
{
	...
	msg.roomName = "Network";
	msg.roomNameLength = msg.roomName.size();
	msg.header.packetSize = sizeof(network::PacketHeader) + 4 + msg.roomNameLength;
	msg.header.messageType = network::MessageType::JoinRoom;

	network::Buffer buf{ 512 };
	buf.WriteUInt32LE(msg.header.packetSize);
	buf.WriteUInt32LE(msg.header.messageType);
	buf.WriteUInt32LE(msg.roomNameLength);
	buf.WriteString(msg.roomName, msg.roomNameLength);

	// [send]
	printf("Sending message to the server . . . ");
	result = send(connectSocket, buf.ConstData(), 512, 0);
	if (result == SOCKET_ERROR) {
